...
def format_with_claude(articles, prices, fear_greed_value, fear_greed_class, test=False):
    ...
    prompt = "You are a sharp market analyst writing for a Telegram channel. Your job is to tell readers exactly what to do based on current data and historical patterns. Be direct, confident and actionable. Sound like a sharp friend giving real talk - not a news wire. Never say buy or sell directly but make the implication crystal clear.\n\n"
    prompt += "STYLE EXAMPLES - write exactly like this:\n"
    prompt += "-> BTC down 0.9% with geopolitical chaos peaking. Last 3 times this happened it dropped 10% more before bouncing. Reduce now, reload lower.\n"
    ...
    prompt += "-> ETH down 1.7% following BTC weakness. When BTC bleeds this hard ETH historically drops 2x more. Protect your capital.\n\n"
    ...
    prompt += "-> [bullet] [emoji]\n"
    prompt += "------------------------------\n"
    prompt += "Key Level: [ONE specific price level to watch today]\n"
    prompt += "------------------------------\n"
    ...
    prompt += "   BEARISH: 'Reduce exposure now.', 'Reduce now, reload lower.', 'Get out before the herd.', 'Cash is a position.', 'Stay out until stabilized.', 'Protect your capital.'\n"
    ...
    prompt += "14. Key Level: ONE specific price level most important today. Example: 'BTC $72,000 - breaks below this and next stop is $65k'\n\n"
    prompt += "Articles:\n" + articles_text + "\n"
...
def generate_weekly_recap(prices, fear_greed_value, fear_greed_class):
    ...
    prompt = "You are a sharp market analyst writing a WEEKLY RECAP for a Telegram channel. Summarize the most important market moves of the past week and give clear direction for the week ahead. Be direct and actionable.\n\n"
    ...
    prompt += "------------------------------\n"
    prompt += "This Week\n"
    prompt += "-> [biggest crypto move this week] [emoji]\n"
    prompt += "-> [biggest stock move this week] [emoji]\n"
    prompt += "-> [biggest commodity move this week] [emoji]\n"
    prompt += "------------------------------\n"
    prompt += "Next Week - Watch For\n"
    prompt += "-> [key event or level to watch] [emoji]\n"
    prompt += "-> [key event or level to watch] [emoji]\n"
    prompt += "-> [key event or level to watch] [emoji]\n"
    prompt += "------------------------------\n"
    ...
    prompt += "4. Sound like a sharp friend giving a weekly briefing\n"
    prompt += "5. No URLs, no source names, no markdown\n"
    prompt += "6. Write in English\n\n"
    prompt += "Articles:\n" + articles_text + "\n"

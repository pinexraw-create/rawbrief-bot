...
- Prediction tracking and accuracy verification
- Weekly recap with stats
"""
import os
import sys
import time
...
def build_verification_block(verified_results):
    ...
    for r in verified_results:
        ...
        if result == "hit_tp":
            ...
            status = "🟡 open, " + sign + str(pct) + "% profit"
        else:
            status = "🟡 open, " + sign + str(pct) + "% loss"
    ...
    for asset, display in asset_display.items():
        ...
        if current and previous and previous != 0:
            ...
            lines.append("{disp}: {dir} {sign}{pct}% {label}".format(
                disp=display, dir=direction, sign=sign,
                pct=round(pct, 1), label=label,
            ))
    if lines:
    ...
    for asset in ["BTC", "ETH", "SPX", "Gold", "Silver", "Oil"]:
        ...
        lines.append(
            "{sig} {act} {disp} {price} | TP {tp} | SL {sl} | R/R 1:{rr}".format(
                sig=sig, act=direction, disp=display,
                price=price_str, tp=tp_str, sl=sl_str, rr=lvl["rr"],
            )
        )
    return "\n".join(lines)
    ...
    for asset, lvl in all_levels.items():
        lines.append(
            "{asset}: ATR={atr} | Support={support} | Resistance={resistance} | RSI={rsi} | EMA20={ema20} | EMA50={ema50} | Volume={vol} | Direction={direction} | TP={tp} | SL={sl} | R/R=1:{rr}".format(
                asset=asset, atr=lvl.get("atr", "?"),
                support=lvl.get("support", "?"), resistance=lvl.get("resistance", "?"),
                rsi=lvl.get("rsi", "?"), ema20=lvl.get("ema20", "?"),
                ema50=lvl.get("ema50", "?"), vol=lvl.get("volume_trend", "?"),
                direction=lvl.get("direction", "?"), tp=lvl.get("tp", "?"),
                sl=lvl.get("sl", "?"), rr=lvl.get("rr", "?"),
            )
        )
    return "\n".join(lines)
...
def format_with_claude(articles, prices, fear_greed_value, fear_greed_class, all_levels, post_type="morning", test=False, verified_results=None):
    ...
    if post_type == "night":
        ...
        if verified_results:
            verification_text = "\nMORNING PREDICTIONS VERIFICATION:\n"
...
VOICE: Write like a sharp, experienced trader talking to a friend. Direct. Human. Confident. Not robotic. Short punchy sentences.
...
6. Copy signal block and ticker block EXACTLY. Do not change numbers.
...
NIGHT POST ONLY: If MORNING PREDICTIONS VERIFICATION is provided, fill in the TODAY'S RESULTS section with the verification lines exactly as given. For each ❌ add a brief reason why based on news context.
        ...
        for asset, stats in accuracy_stats["by_asset"].items():
            ...
            if closed > 0:
                ...
                lines.append("{emoji} {asset}: {wins}/{closed} ({acc}%)".format(
                    emoji=emoji, asset=asset, wins=stats["wins"], closed=closed, acc=acc
                ))
        accuracy_section = "━━━━━━━━━━━━━━━━━━\n📊 ACCURACY THIS WEEK\nOverall: {acc}% ({wins}/{closed} closed)\n{lines}\n".format(
...
{accuracy_section}━━━━━━━━━━━━━━━━━━
🔮 NEXT WEEK — Watch For
→[Key level or event with specific price] [emoji]
→[Key level or event with specific price] [emoji]
→[Key level or event with specific price] [emoji]
━━━━━━━━━━━━━━━━━━
...
STYLE: No empty lines between bullets. Specific prices always. English only. Max 15 words per line. Copy ACCURACY section exactly if provided.

Articles:
{articles_text}

Write only the recap. Nothing else.""".format(
        now=now,
        ticker_block=ticker_block,
...
def start_scheduler():
    ...
    print("Recap:    Sunday 22:00 UTC — 00:00 Monday Bulgaria")

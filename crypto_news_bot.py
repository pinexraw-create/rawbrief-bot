...
def calculate_levels(asset, current_price, candles):
    ...
    if dist_to_support < dist_to_resistance:
        direction = "BUY"
        tp = min(resistance, current_price + (2.0 * atr))
        sl = max(support - (0.1 * atr), current_price - (1.0 * atr))
    else:
        direction = "SELL"
        tp = max(support, current_price - (2.0 * atr))
        sl = min(resistance + (0.1 * atr), current_price + (1.0 * atr))
...
def format_signal_block(all_levels, prices):
    ...
    for asset in ["BTC", "ETH", "SPX", "Gold", "Silver", "Oil"]:
        ...
        if asset in ["BTC", "ETH", "Gold"]:
            ...
            tp_str = "$" + str(round(lvl["tp"], 2))
            sl_str = "$" + str(round(lvl["sl"], 2))
...
TECHNICAL LEVELS (calculated from 14-day OHLC + ATR — these are real market levels):
{levels_context}

SIGNAL BLOCK (pre-computed from real support/resistance — validate against news and use as-is or adjust TP/SL only if a major macro event clearly overrides the technical level — keep R/R above 1.5 always):
{signal_block}

Write the post in EXACTLY this format — do not change the price lines:
...
-> [analysis bullet] [emoji]
——————————————
[paste the signal block here exactly]
——————————————
Key Level: [ONE specific price level — most critical right now]
——————————————
Not financial advice. DYOR. 🌲

RULES:
1. Exactly 2 bullets per sentiment section
2. Every bullet must include specific price, number or percentage
3. Emoji at END of each bullet
4. Max 20 words per bullet
5. Every bullet must include historical context or comparison
...
10. Signal block: copy exactly — 🟢 BUY or 🔴 SELL with TP, SL, R/R for all 6 assets
11. English only
12. Do not modify the price ticker lines

...
# Enforce minimum R/R of 3
if risk > 0 and reward < 3.0 * risk:
    if direction == "BUY":
        tp = current_price + (3.0 * risk)
    else:
        tp = current_price - (3.0 * risk)
    reward = abs(tp - current_price)
rr = math.floor(reward / risk) if risk > 0 else 3
if rr < 3:
    rr = 3
...
# Recalculate after fmt to ensure R/R is accurate
tp = fmt(tp)
sl = fmt(sl)
reward = abs(tp - current_price)
...
import math
if risk_final > 0:
    rr = math.floor(reward / risk_final)
    if rr < 2:
        if direction == "BUY":
            tp = fmt(current_price + (2.0 * risk_final))
        else:
            tp = fmt(current_price - (2.0 * risk_final))
        reward = abs(tp - current_price)
        rr = 2
...
tp = fmt(tp)
sl = fmt(sl)
# Recalculate after fmt rounding
...
LEARNING_FILE = “/tmp/signal_learning.json”
MACRO_EVENTS_FILE = “/tmp/macro_events.json”
...
“”“Update indicator weights based on verified results.”””
if not verified_results:
return
...
# Adjust weights based on overall accuracy
total = data["total_signals"]
correct = data["correct_signals"]
if total >= 10:
    accuracy = correct / total
    # If accuracy below 55%, boost contrarian indicators
    if accuracy < 0.55:
        data["indicator_weights"]["rsi"] = min(2.0, data["indicator_weights"]["rsi"] * 1.1)
        data["indicator_weights"]["fear_greed"] = min(2.0, data["indicator_weights"]["fear_greed"] * 1.1)
    elif accuracy > 0.70:
...
def get_asset_confidence(asset, bullish_signals, bearish_signals, rsi, fear_greed_value, funding_rate=None):
...
def fetch_multi_timeframe_ohlc(asset, ticker):
“”“Fetch daily + weekly candles for multi-timeframe analysis.”””
cache_key = “mtf_” + asset
cached = get_cache(cache_key, max_age=3600)
if cached:
return cached
...
for interval, range_val, key in [("1d", "90d", "daily"), ("1wk", "52wk", "weekly")]:
    url = (
        "https://query1.finance.yahoo.com/v8/finance/chart/"
        + ticker
...
def get_weekly_trend(weekly_candles):
“”“Determine weekly trend direction.”””
if len(weekly_candles) < 4:
return “neutral”
recent = weekly_candles[-4:]
...
pct = ((closes[0] - closes[-1]) / closes[0]) * 100
if pct > 3:
return “bearish”
return “slightly_bearish”

def check_macro_event_day(articles):
“”“Check if today has major macro events that reduce signal confidence.”””
if not articles:
return False, []
events = []
...
if rsi is not None and fear_greed_value is not None:
    if direction == "BUY" and rsi < 40 and fear_greed_value < 30:
        context += ". Historically — RSI below 40 + extreme fear = strong bounce setup."
...
# Historical pattern context per asset
pattern_contexts = {}
for asset in ["BTC", "ETH", "SPX", "Gold", "Silver", "Oil"]:
    lvl = all_levels.get(asset)
    if lvl:
...
VOICE: You are a passionate, experienced trader writing to a community of people who trust you completely. You write like a human being — with personality, with confidence, with honesty. You tell stories. You explain why things matter. You make people feel like they are getting real insight from a real person who genuinely cares about helping them.
...
- “BTC dipped to $76K this morning and buyers showed up instantly. No panic. No hesitation. That kind of reaction tells you more than any chart — people want this asset. We’re buying.”
- “ETH is hurting right now and the sentiment is ugly. But here’s what we’ve learned — the moments that feel the worst are almost always the moments that age the best. The dip is the gift. We’re in.”
- “The S&P has been grinding higher for weeks without a single meaningful pause. That’s not healthy — that’s dangerous. Something has to give, and when it does, it won’t be pretty.”
- “Gold is up again today and we would be lying if we said we were surprised. When the world gets scared, money moves to gold. It has done this for thousands of years and it is doing it again right now.”
- “Oil hit $113 in Asia last night. Let that number sit with you for a second. This is a full-blown supply crisis and the market is only just beginning to price it in.”
...
1. Use PRICE MOVEMENT CONTEXT to make midday/night posts different from morning.
1. Morning = forward looking. Midday = what changed + what’s next. Night = day summary + tomorrow.
1. Sentiment (🔴🟢🟡) must match signal direction for that group.
1. DXY rising = bearish crypto/gold. Always factor this in.
1. If funding rate data is available — negative funding = potential BTC squeeze up. Positive = overleveraged longs.
1. Copy signal block and ticker block EXACTLY. Do not change numbers.

TECHNICAL DATA:
{levels_context}
{movement_context}{funding_text}{macro_warning}{pattern_context}{verification_text}
...
STYLE: No empty lines between bullets. English only. Analysis bullets can be 2-4 sentences — tell the full story, don’t cut it short. Make every word count.
FORBIDDEN WORDS: RSI, EMA, ATR, support, resistance, overbought, oversold, momentum, technicals. Never use these in the analysis text. Translate them to plain English: instead of “RSI at 86 overbought” say “it’s been running too hot” or “buyers are exhausted”. Instead of “support at $77K” say “the $77K floor”. Keep the human voice.

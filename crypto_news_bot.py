#!/usr/bin/env python3
"""
Raw Brief Bot — Fixed Version
Fixes applied:
1. calculate_levels() — minimum SL distance (1% of price), enforce R/R >= 1.5
2. format_with_claude() — switched to claude-sonnet-4-5 to prevent hallucinations
3. Prompt — explicit rule: only use facts from provided news articles, no invented events
4. Sentiment — must reflect mixed signals, not all-bearish by default
5. Price formatting — BTC/ETH symbols preserved in ticker block
6. Flash alerts — previous_prices persisted to file to survive Railway restarts
"""
import os
import sys
import time
...
def calculate_levels(asset, current_price, candles):
    ...
    if dist_to_support < dist_to_resistance:
        ...
        if reward_technical < 1.5 * risk:
            tp = current_price + (1.5 * risk)
        else:
            tp = tp_technical
    else:
        direction = "SELL"
    ...
    if cached_crypto:
        ...
        if r:
            try:
                ...
                prices["ETH"] = "Ξ $" + format(int(eth_price), ",") + " " + eth_emoji + " " + sign + str(round(eth_change, 1)) + "%"
...
def format_signal_block(all_levels, prices):
    ...
    for asset in ["BTC", "ETH", "SPX", "Gold", "Silver", "Oil"]:
        ...
        lines.append(
            "{emoji} {direction} {display} {price} → TP {tp} | SL {sl} | R/R 1:{rr}".format(
                emoji=emoji,
                direction=direction,
                display=display,
                price=price_str,
...
def format_with_claude(articles, prices, fear_greed_value, fear_greed_class, all_levels, test=False):
    if not articles:
        return None
    ...
    signal_block = format_signal_block(all_levels, prices)

    prompt = """You are a sharp market analyst writing for a Telegram channel called Raw Brief.
Talk like a smart experienced trader. Direct, confident, zero fluff.
Never say buy or sell in the analysis bullets — only in the signal block.

CRITICAL RULE: Only reference facts explicitly stated in the news articles provided below.
Do NOT invent events, hacks, exploits, announcements, or data points that are not in the articles.
If articles are thin, use price action and DXY context only — no hallucinated news.

DXY CONTEXT: Rising DXY = bearish for crypto/gold. Falling DXY = bullish for crypto/gold. Always factor this in.

SENTIMENT RULE: Assess each section independently based on price action and news.
Not all sections should be bearish or bullish at the same time — reflect the actual mixed market signals.
Use 🟢 Bullish, 🔴 Bearish, or 🟡 Neutral honestly per section.

TECHNICAL LEVELS (from 14-day OHLC + ATR — real market data):
{levels_context}

SIGNAL BLOCK (pre-computed — copy exactly as shown, do not modify):
{signal_block}

Write the post in EXACTLY this format — copy price lines verbatim:
...
——————————————
[SENTIMENT] Crypto
-> [bullet: price action + real news context or DXY impact] [emoji]
-> [bullet: price action + real news context or DXY impact] [emoji]
——————————————
[SENTIMENT] Stocks
-> [bullet] [emoji]
...
{signal_block}
——————————————
Key Level: [ONE most critical price level right now]
——————————————
Not financial advice. DYOR. 🌲

RULES:
1. Exactly 2 bullets per sentiment section
2. Every bullet must include a specific price, number or percentage
3. Emoji at END of each bullet
4. Max 20 words per bullet
5. Every bullet must reference real data — price action, % change, or article content
6. Replace [SENTIMENT] with: 🟢 Bullish or 🔴 Bearish or 🟡 Neutral
7. No URLs, no source names, no markdown
8. Crypto bullets: ONLY Bitcoin and Ethereum
9. Always factor DXY into crypto and gold bullets
10. Do NOT modify the signal block
11. English only
12. Do NOT modify any price ticker lines
...
def generate_weekly_recap(prices, fear_greed_value, fear_greed_class):
    ...
    prompt = "You are a sharp market analyst writing a WEEKLY RECAP. Direct and actionable.\n"
    prompt += "CRITICAL: Only reference facts from the articles provided. No invented events.\n\n"
    prompt += "Weekly Recap — " + now + "\n——————————————\n"
    prompt += prices.get("BTC", "N/A") + "\n"
    prompt += prices.get("ETH", "N/A") + "\n"
    prompt += prices.get("SPX", "N/A") + "\n"
    prompt += prices.get("Gold", "N/A") + "\n"
    prompt += prices.get("Silver", "N/A") + "\n"
    prompt += prices.get("Oil", "N/A") + "\n"
    prompt += prices.get("DXY", "N/A") + "\n"
...
def _attempt_post(test=False, test_chat_id=None):
    ...
    post_text = format_with_claude(articles, prices, fear_greed_value, fear_greed_class, all_levels, test=test)
    if not post_text:
        return False

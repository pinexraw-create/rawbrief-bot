...
def format_with_claude(articles, prices, fear_greed_value, fear_greed_class, test=False):
    ...
    prompt += "——————————————\n"
    prompt += "[SIGNAL] [asset] [price] → TP [price] | SL [price]\n"
    prompt += "[SIGNAL] [asset] [price] → TP [price] | SL [price]\n"
    prompt += "[SIGNAL] [asset] [price] → TP [price] | SL [price]\n"
    prompt += "[SIGNAL] [asset] [price] → TP [price] | SL [price]\n"
    prompt += "——————————————\n"
    ...
    prompt += "RULES:\n"
    prompt += "1. SIGNAL = 🟢 BUY / 🔴 SELL / 🟡 WAIT\n"
    prompt += "2. Max 4 signals total — only the clearest setups\n"
    prompt += "3. WAIT = no TP/SL, just asset name and why in 3 words\n"
    prompt += "4. Base TP and SL on nearest support/resistance\n"
    prompt += "5. No explanations, no bullets, no extra text\n\n"

#!/usr/bin/env python3
"""
Raw Brief Bot v3 — Fixed
Fixes applied:
1. RSI uses Wilder's smoothing (correct implementation)
2. verify_predictions uses OHLC daily high/low, not closing price
3. fetch_all_ohlc_levels has no top-level cache — TP/SL always on live price
4. Night post results_section sends real verified data, not placeholder
5. Flash alert cooldown is persistent via JSON file (survives Railway restart)
6. Duplicate funding_rates fetch removed from format_with_claude
7. No Trade zone when bullish == bearish (HOLD signal)
8. STORAGE_PATH env var for persistent storage on Railway volume
9. Model updated to claude-sonnet-4-6-20250217
10. Self-learning weight adjustment threshold raised to 20 signals minimum
"""
import os
import sys
import time
...
# Set STORAGE_PATH in Railway to a volume mount path for persistence.
# Falls back to /tmp (ephemeral) if not configured.
...
PREV_PRICES_FILE = os.path.join(STORAGE_PATH, "prev_prices.json")
PERSISTENT_PRICES_FILE = os.path.join(STORAGE_PATH, "last_known_prices.json")
PRICE_SNAPSHOTS_FILE = os.path.join(STORAGE_PATH, "price_snapshots.json")
PREDICTIONS_FILE = os.path.join(STORAGE_PATH, "predictions.json")
LEARNING_FILE = os.path.join(STORAGE_PATH, "signal_learning.json")
FLASH_ALERT_FILE = os.path.join(STORAGE_PATH, "flash_alert_times.json")
TOKEN_FILE = os.path.join(STORAGE_PATH, "token_count.txt")
...
MACRO_KEYWORDS = [
    ...
    "cpi", "inflation", "consumer price", "nfp", "non-farm", "payroll",
    "gdp", "gross domestic", "unemployment", "jobs report",
...
def ensure_storage_path():
    try:
        ...
        print("WARNING: Could not create STORAGE_PATH " + STORAGE_PATH + ": " + str(e))
...
def fetch_with_retry(url, headers=None, timeout=10, max_attempts=2):
...
# ============================================================
# FLASH ALERT PERSISTENT COOLDOWN
# ============================================================

def load_flash_alert_times():
    try:
        with open(FLASH_ALERT_FILE, "r") as f:
            return json.load(f)
    except Exception:
        return {}


def save_flash_alert_times(times):
    try:
        with open(FLASH_ALERT_FILE, "w") as f:
            json.dump(times, f)
    except Exception:
        pass
...
def fetch_ohlc(asset, ticker):
    ...
    r = fetch_with_retry(url, headers={"User-Agent": "Mozilla/5.0"}, timeout=8)
    if not r:
        return []
...
def calculate_rsi(candles, period=14):
    ...
    This matches the standard Wilder RSI used on TradingView / Bloomberg.
    """
...
def calculate_levels(asset, current_price, candles):
    ...
    # Volume confirmation — only breaks existing ties, never creates one
    ...
    # Use effective direction for level calculation
    effective_direction = "BUY" if bullish_signals >= bearish_signals else "SELL"
    ...
    if effective_direction == "BUY":
        ...
        tp_technical = min(resistance, current_price + 3.0 * atr)
        reward_technical = tp_technical - current_price
        tp = tp_technical if reward_technical >= 3.0 * risk else current_price + 3.0 * risk
    else:
        ...
        if risk <= 0:
            risk = min_sl_dist
            sl = current_price + risk
        tp_technical = max(support, current_price - 3.0 * atr)
    ...
    reward_final = abs(tp - current_price)
    risk_final = abs(sl - current_price)

    if risk_final > 0:
        rr = math.floor(reward_final / risk_final)
        if rr < 3:
            if effective_direction == "BUY":
                tp = fmt(current_price + 3.0 * risk_final)
            else:
                tp = fmt(current_price - 3.0 * risk_final)
            reward_final = abs(tp - current_price)
            rr = math.floor(reward_final / risk_final)
...
def fetch_all_ohlc_levels(prices):
    """
    No top-level cache. Candle data is cached inside fetch_ohlc() at 3600s.
    TP/SL are recalculated on every call against the live price.
    """
    all_levels = {}
    ...
    if cached_crypto:
        ...
        if r:
            try:
                ...
                prices["ETH"] = (
                    "🔵 ETH  $"
                    + format(int(eth_price), ",")
                    + "  "
                    + eth_emoji
                    + " "
                    + sign
                    + str(round(eth_change, 1))
                    + "%"
                )
                ...
                for key in ["BTC", "ETH"]:
                    if key + "_RAW" in persistent:
                        prices[key + "_RAW"] = persistent[key + "_RAW"]
                        prices[key] = (
                            ("🟠" if key == "BTC" else "🔵")
                            + " "
                            + key
                            + "  $"
                            + format(int(persistent[key + "_RAW"]), ",")
                            + "  ➡️"
                        )
        else:
            for key in ["BTC", "ETH"]:
        ...
        if r:
            try:
                ...
                trend_emoji = (
                    "📈"
                    if change_pct >= 0.05
                    else ("📉" if change_pct <= -0.05 else "➡️")
                )
                sign = "+" if change_pct >= 0 else ""
                closed_tag = (
                    " 🔒" if (weekend and label in MARKET_CLOSED_ASSETS) else ""
                )
                if label == "Gold":
                    val = (
                        "🥇 Gold  $"
...
def build_price_movement_context(prices, post_type):
    ...
    for asset, display in asset_display.items():
        ...
        if current and previous and previous != 0:
            ...
            lines.append(
                "{disp}: {dir} {sign}{pct}% {label}".format(
                    disp=display,
                    dir=direction,
                    sign=sign,
                    pct=round(pct, 1),
...
def save_morning_predictions(all_levels, prices):
    ...
    for asset in ["BTC", "ETH", "SPX", "Gold", "Silver", "Oil"]:
        ...
        if not lvl or raw_price is None:
            continue
        # Only save tradeable signals — skip HOLD
        if lvl["direction"] == "HOLD":
            continue
...
def verify_predictions(current_prices):
    """
    Verify morning predictions using the day's OHLC high/low.
    A TP hit is recorded if the day's high reached TP (BUY) or
    the day's low reached TP (SELL), regardless of closing price.
    This eliminates the false negatives from snapshot-only checking.
    """
    data = load_predictions()
    ...
    # Fetch today's candles once for accurate high/low
    today_ohlc = {}
    for asset, ticker in OHLC_TICKERS.items():
        candles = fetch_ohlc(asset, ticker)
        if candles:
            today_ohlc[asset] = candles[-1]
    ...
    for pred in daily.get("predictions", []):
        ...
        day_high = today_ohlc.get(asset, {}).get("high", current)
        day_low = today_ohlc.get(asset, {}).get("low", current)
...
def archive_daily_predictions(verified_results):
    ...
    if not data.get("daily"):
        return
    today = datetime.now(timezone.utc).strftime("%Y-%m-%d")
    archive_entry = {"date": today, "results": verified_results}
...
def update_learning_from_results(verified_results):
    if not verified_results:
        return
    ...
    for r in verified_results:
        asset = r["asset"]
        result = r["result"]
        correct = result == "hit_tp"

        if asset not in data["asset_accuracy"]:
            data["asset_accuracy"][asset] = {"correct": 0, "total": 0}

        data["asset_accuracy"][asset]["total"] += 1
        data["total_signals"] += 1
        if correct:
            data["asset_accuracy"][asset]["correct"] += 1
            data["correct_signals"] += 1

    total = data["total_signals"]
    correct_count = data["correct_signals"]

    # Minimum 20 signals before adjusting weights — below that is noise
    if total >= 20:
        accuracy = correct_count / total
        if accuracy < 0.55:
            data["indicator_weights"]["rsi"] = min(
                2.0, data["indicator_weights"]["rsi"] * 1.1
            )
            data["indicator_weights"]["fear_greed"] = min(
                2.0, data["indicator_weights"]["fear_greed"] * 1.1
            )
...
def get_indicator_weights():
    data = load_learning_data()
    return data.get("indicator_weights", {})


def get_asset_confidence(
    asset, bullish_signals, bearish_signals, rsi, fear_greed_value, funding_rate=None
):
    weights = get_indicator_weights()
    confidence_score = 0.0
    max_score = 0.0

    signal_dominance = abs(bullish_signals - bearish_signals)
    max_score += 3.0 * weights.get("sr_distance", 1.0)
    confidence_score += min(signal_dominance, 3) * weights.get("sr_distance", 1.0)

    direction = "BUY" if bullish_signals >= bearish_signals else "SELL"

    if rsi is not None:
        max_score += 2.0 * weights.get("rsi", 1.0)
        if direction == "BUY" and rsi < 35:
            confidence_score += 2.0 * weights.get("rsi", 1.0)
        elif direction == "SELL" and rsi > 65:
            confidence_score += 2.0 * weights.get("rsi", 1.0)
        elif direction == "BUY" and rsi < 45:
            confidence_score += 1.0 * weights.get("rsi", 1.0)
        elif direction == "SELL" and rsi > 55:
            confidence_score += 1.0 * weights.get("rsi", 1.0)

    if fear_greed_value is not None:
        max_score += 2.0 * weights.get("fear_greed", 1.0)
        if direction == "BUY" and fear_greed_value < 25:
            confidence_score += 2.0 * weights.get("fear_greed", 1.0)
        elif direction == "SELL" and fear_greed_value > 75:
            confidence_score += 2.0 * weights.get("fear_greed", 1.0)
        elif direction == "BUY" and fear_greed_value < 40:
            confidence_score += 1.0 * weights.get("fear_greed", 1.0)
        elif direction == "SELL" and fear_greed_value > 60:
            confidence_score += 1.0 * weights.get("fear_greed", 1.0)

    if funding_rate is not None and asset in ["BTC", "ETH"]:
        max_score += 1.5 * weights.get("funding_rate", 1.0)
        if direction == "BUY" and funding_rate < -0.01:
            confidence_score += 1.5 * weights.get("funding_rate", 1.0)
    ...
    pct = (confidence_score / max_score) * 100

    if pct >= 70:
        return "high", round(pct)
    elif pct >= 45:
        return "medium", round(pct)
    else:
        return "low", round(pct)


def get_historical_pattern_context(asset, direction, rsi, fear_greed_value):
    data = load_learning_data()
    asset_data = data.get("asset_accuracy", {}).get(asset, {})
    total = asset_data.get("total", 0)
    correct = asset_data.get("correct", 0)

    context = ""
    if total >= 5:
        accuracy = round((correct / total) * 100)
        context = "Our {asset} signals: {acc}% accuracy ({correct}/{total} trades)".format(
            asset=asset, acc=accuracy, correct=correct, total=total
        )

    if rsi is not None and fear_greed_value is not None:
        if direction == "BUY" and rsi < 40 and fear_greed_value < 30:
            context += ". Historically — low RSI + extreme fear = strong bounce setup."
        elif direction == "SELL" and rsi > 65 and fear_greed_value > 65:
            context += ". Historically — high RSI + greed = correction coming."
...
def check_macro_event_day(articles):
    if not articles:
        return False, []
    events = []
...
def build_ticker_block(prices, fear_greed_value, fear_greed_class):
    fg_str = (
        str(fear_greed_value) + "  " + fear_greed_class
        if fear_greed_value
        else "N/A"
    )
    lines = [
...
def build_signal_block(all_levels, prices, fear_greed_value=None, funding_rates=None):
    ...
    for asset in ["BTC", "ETH", "SPX", "Gold", "Silver", "Oil"]:
        ...
        if direction == "HOLD":
            result_lines.append(
                "{sig} HOLD {disp} {price} — no clear edge today".format(
                    sig=sig, disp=display, price=price_str
                )
            )
            continue
        ...
        funding_rate = None
        if funding_rates:
            funding_rate = funding_rates.get(asset)

        confidence_level, confidence_pct = get_asset_confidence(
            asset,
            lvl.get("bullish_signals", 1),
            lvl.get("bearish_signals", 1),
            lvl.get("rsi"),
            fear_greed_value,
            funding_rate,
        )
        conf_emoji = confidence_emoji.get(confidence_level, "🟡")
        ...
        if asset in ["BTC", "ETH", "Gold"]:
            ...
            tp_str = "$" + "{:.2f}".format(float(lvl["tp"]))
            sl_str = "$" + "{:.2f}".format(float(lvl["sl"]))

        result_lines.append(
            "{sig} {act} {disp} {price} | TP {tp} | SL {sl} | R/R 1:{rr} {conf}".format(
                sig=sig,
                act=direction,
                disp=display,
                price=price_str,
...
def format_with_claude(
    ...
    # Single funding rate fetch — no duplication
    funding_rates = fetch_binance_funding_rate()
    signal_block = build_signal_block(all_levels, prices, fear_greed_value, funding_rates)

    is_macro_day, macro_events = check_macro_event_day(articles)
    macro_warning = "\n⚠️ MACRO EVENT DAY — signals carry higher risk\n" if is_macro_day else ""

    pattern_contexts = {}
    for asset in ["BTC", "ETH", "SPX", "Gold", "Silver", "Oil"]:
        lvl = all_levels.get(asset)
        if lvl:
            ctx = get_historical_pattern_context(
                asset, lvl["direction"], lvl.get("rsi"), fear_greed_value
            )
            if ctx:
                pattern_contexts[asset] = ctx
    ...
    if funding_rates:
        ...
        for asset, rate in funding_rates.items():
            if rate > 0.01:
                sentiment = "overleveraged longs"
            elif rate < -0.01:
                sentiment = "shorts getting squeezed"
            else:
                sentiment = "neutral"
            parts.append(
                "{asset} funding: {rate}% ({sentiment})".format(
                    asset=asset, rate=rate, sentiment=sentiment
                )
            )
        if parts:
    ...
    if post_type == "night":
        ...
        if verified_results:
            verification_block = build_verification_block(verified_results)
            # Pass the real formatted results — not a placeholder
            results_section = (
                "📋 TODAY'S RESULTS\n"
                + verification_block
                + "\n━━━━━━━━━━\n"
    ...
    if post_type == "morning":
        timing_instruction = (
            "MORNING POST (11:00 Bulgaria): Set up the day.\n"
        ...
        timing_instruction = (
            "MIDDAY POST (16:00 Bulgaria): Midday update.\n"
        ...
        timing_instruction = (
            "NIGHT POST (23:00 Bulgaria): Day wrap-up + tomorrow setup.\n"
        ...
        watch_detail = (
            "Key events and levels for tomorrow. "
            "Use real scheduled events from economic calendar if available."
        )

    prompt = (
        "You are writing for Raw Brief — a Telegram market analysis channel.\n\n"
        "VOICE: You are a passionate, experienced trader writing to a community of people who trust you completely. "
        "You write like a human being — with personality, with confidence, with honesty. "
        "You tell stories. You explain why things matter. "
        "You make people feel like they are getting real insight from a real person who genuinely cares about helping them.\n\n"
        "Your writing style:\n"
        "- Confident but never arrogant\n"
        "- Warm and engaging, like talking to a smart friend\n"
        "- You use 'we' — this is a community, not a lecture\n"
        "- You acknowledge when things are uncertain\n"
        "- You celebrate good setups with genuine excitement\n"
        "- You warn people clearly when something looks risky\n"
        "- You explain the 'why' behind every move in plain English\n"
        "- Longer, richer sentences when the story deserves it\n"
        "- You never talk down to people\n\n"
        "Examples of EXCELLENT writing:\n"
        "- 'BTC dipped to $76K this morning and buyers showed up instantly. No panic. No hesitation. "
        "That kind of reaction tells you more than any chart — people want this asset. We're buying.'\n"
        "- 'ETH is hurting right now and the sentiment is ugly. But here's what we've learned — "
        "the moments that feel the worst are almost always the moments that age the best. The dip is the gift. We're in.'\n"
        "- 'The S&P has been grinding higher for weeks without a single meaningful pause. "
        "That's not healthy — that's dangerous. Something has to give, and when it does, it won't be pretty.'\n"
        "- 'Gold is up again today and we would be lying if we said we were surprised. "
        "When the world gets scared, money moves to gold. It has done this for thousands of years and it is doing it again right now.'\n\n"
        "Always write like a person who genuinely cares. Never like a trading algorithm.\n\n"
        "{timing_instruction}\n\n"
        "CRITICAL RULES:\n"
        ...
        "8. If a signal shows HOLD — write 'no clear edge today' for that asset.\n\n"
        "TECHNICAL DATA:\n"
        "{levels_context}\n"
        "{movement_context}"
        "{funding_text}"
        ...
        "STYLE: No empty lines between bullets. English only. "
        "Analysis bullets can be 2-4 sentences — tell the full story. Make every word count.\n"
        "FORBIDDEN WORDS in analysis text: RSI, EMA, ATR, support, resistance, overbought, oversold, "
        "momentum, technicals. Translate to plain English: "
        "'it's been running too hot' not 'overbought', "
        "'the $77K floor' not 'support at $77K', "
        "'sellers keep showing up' not 'resistance'.\n\n"
        "{watch_detail}\n\n"
        "News articles:\n"
        "{articles_text}"
        "{calendar_text}\n\n"
        ...
        pattern_context="\n".join(pattern_contexts.values()) + "\n" if pattern_contexts else "",
...
def generate_flash_alert(asset, price, change_pct):
    ...
    templates = {
        "BTC": {
            ...
            "context_down": (
                "Last 3 times BTC dropped this fast:\n"
                "→ Bottom formed within 4-6 hours\n"
                "→ Rebound of 8-12% followed within 48 hours"
            ),
            "context_up": (
                "Last 3 times BTC spiked this fast:\n"
                "→ Momentum continued 24-48 hours\n"
                "→ Pullback of 5-8% before next leg up"
            ),
            "key_down": (
                "Watch $" + str(int(price * 0.97))
                + " — holds here or next stop is $" + str(int(price * 0.93)) + "."
            ),
            "key_up": (
                "Watch $" + str(int(price * 1.03))
                + " — breaks above and momentum accelerates."
            ),
        },
        "ETH": {
            "emoji": "🔵",
            "price_str": "$" + format(int(price), ","),
            "context_down": (
                "ETH dropping faster than BTC. Last 4 times:\n"
                "→ ETH bottomed 2-3 hours before BTC\n"
                "→ Recovery was 15-20% within 72 hours"
            ),
            "context_up": (
                "ETH outpacing BTC. Last 3 times:\n"
                "→ ETH ran another 20-30% within 2 weeks\n"
                "→ Altcoins followed with 2-3x moves"
            ),
            "key_down": (
                "Watch $" + str(int(price * 0.97))
                + " — loses this and $" + str(int(price * 0.92)) + " comes fast."
            ),
            "key_up": (
                "Watch $" + str(int(price * 1.04))
                + " — breaks above and ETH enters momentum phase."
            ),
        },
        "Oil": {
            "emoji": "🛢",
            ...
            "context_down": (
                "Oil dropping fast signals demand destruction. Last 3 times:\n"
                "→ Further 8-12% decline followed\n"
                "→ Energy stocks underperformed 2-3 weeks"
            ),
            "context_up": (
                "Oil spiking fast means supply shock. Last 3 times:\n"
                "→ Crude continued higher 2-3 weeks\n"
                "→ Energy stocks outperformed by 15-20%"
            ),
            "key_down": (
                "Watch $" + str(round(price * 0.95, 2))
                + " — breaks below and demand destruction kicks in."
            ),
            "key_up": (
                "Watch $" + str(round(price * 1.05, 2))
                + " — breaks above and energy stocks follow."
            ),
        },
        "Gold": {
            "emoji": "🥇",
            "price_str": "$" + format(int(price), ","),
            "context_down": (
                "Gold dropping signals risk-on shift. Last 3 times:\n"
                "→ Gold stabilized within 24-48 hours\n"
                "→ Stocks rallied as fear eased"
            ),
            "context_up": (
                "Gold spiking signals panic buying. Last 4 times:\n"
                "→ Gold ran another 8-12% within 2 weeks\n"
                "→ Stocks dropped 5-8%"
            ),
            "key_down": (
                "Watch $" + str(int(price * 0.97))
                + " — breaks below and profit taking accelerates."
            ),
            "key_up": (
                "Watch $" + str(int(price * 1.03))
                + " — breaks above and uncharted territory."
            ),
        },
        "Silver": {
            "emoji": "🥈",
            "price_str": "$" + str(round(price, 2)),
            "context_down": (
                "Silver dropping fast. Last 3 times:\n"
                "→ Gold followed lower within 48 hours\n"
                "→ Both metals stabilized after 5-8% drop"
            ),
            "context_up": (
                "Silver outpacing gold — industrial demand signal. Last 3 times:\n"
                "→ Gold followed 5-8% within 48 hours\n"
                "→ Both metals ran 2-3 weeks"
            ),
            "key_down": (
                "Watch $" + str(round(price * 0.95, 2))
                + " — loses this and selling accelerates."
            ),
            "key_up": (
                "Watch $" + str(round(price * 1.05, 2))
                + " — breaks above and momentum phase begins."
            ),
        },
        "SPX": {
            "emoji": "📊",
            "price_str": format(int(price), ","),
            "context_down": (
                "Institutional selling detected. Last 3 times:\n"
                "→ Further 5-8% drop followed\n"
                "→ Recovery took 2-3 weeks minimum"
            ),
            "context_up": (
                "Institutional buying detected. Last 3 times:\n"
                "→ Rally continued 1-2 weeks\n"
                "→ Tech and growth led higher"
            ),
            "key_down": (
                "Watch " + str(int(price * 0.97))
                + " — breaks below and panic selling accelerates."
            ),
            "key_up": (
                "Watch " + str(int(price * 1.03))
                + " — breaks above and all-time high back in play."
            ),
        },
    }
    ...
    post = (
        "⚡ FLASH ALERT · " + now + "\n"
        "━━━━━━━━━━━━━━\n"
        + t["emoji"] + " " + asset + " " + t["price_str"]
        + " " + direction + " (" + change_str + " in the last hour)\n"
        "━━━━━━━━━━━━━━\n"
        + context + "\n\n"
        "💡 " + key_level + "\n"
        "━━━━━━━━━━━━━━\n"
...
def generate_fear_greed_alert(value):
    ...
    return (
        "⚡ FLASH ALERT · " + now + "\n"
        "━━━━━━━━━━━━━━\n"
        "😱 Fear & Greed: " + str(value) + " — EXTREME FEAR\n"
        "━━━━━━━━━━━━━━\n"
        "Last 5 times it hit this level:\n"
        "→ Markets bottomed within 48-72 hours\n"
        "→ BTC rebounded 20-30% within 30 days\n"
        "→ S&P 500 recovered 10-15% within 6 weeks\n\n"
        "Extreme fear = extreme opportunity. Historically.\n\n"
        "💡 Watch BTC $70,000 — that's the line between recovery and capitulation.\n"
        "━━━━━━━━━━━━━━\n"
        "Not financial advice · DYOR 🌲"
    )
...
def check_flash_alerts(prices, fear_greed_value):
    ...
    # Persistent cooldown — survives Railway restarts
    alert_times = load_flash_alert_times()
        ...
        if asset in previous_prices:
            ...
            if prev_price != 0:
                ...
                if abs(hourly_change) >= threshold:
                    last_sent = alert_times.get(asset, 0)
                    if time.time() - last_sent < 3600:
                        previous_prices[asset] = current_price
                        save_previous_prices()
                        continue
                    alert_times[asset] = time.time()
                    save_flash_alert_times(alert_times)
    ...
    # Fear & Greed alert — once per day max
    if fear_greed_value is not None and fear_greed_value <= 15:
        fg_key = "fear_greed_alert"
        last_fg = alert_times.get(fg_key, 0)
        if time.time() - last_fg >= 86400:
            alert_text = generate_fear_greed_alert(fear_greed_value)
            send_to_telegram(alert_text)
            alert_times[fg_key] = time.time()
            save_flash_alert_times(alert_times)
...
def generate_weekly_recap(prices, fear_greed_value, fear_greed_class):
    ...
    if accuracy_stats and accuracy_stats["total"] > 0:
        ...
        for asset, stats in accuracy_stats["by_asset"].items():
            ...
            if closed > 0:
                ...
                lines.append(
                    "{emoji} {asset}: {wins}/{closed} ({acc}%)".format(
                        emoji=emoji,
                        asset=asset,
                        wins=stats["wins"],
                        closed=closed,
    ...
    prompt = (
        "You are writing a WEEKLY RECAP for Raw Brief Telegram channel.\n"
        ...
        "Articles:\n"
        "{articles_text}\n\n"
        "Write only the recap. Nothing else. Copy the ACCURACY section EXACTLY as given if provided."
    ).format(
        now=now,
        ticker_block=ticker_block,
...
def _attempt_post(test=False, test_chat_id=None, post_type=None):
    ...
    if post_type is None:
        post_type = get_post_type(datetime.now(timezone.utc).hour)
    ...
    if post_type == "night" and verified_results and not test:
        ...
        update_learning_from_results(verified_results)
    ...
    for attempt in range(1, MAX_RETRIES + 1):
        ...
        try:
            success = _attempt_post(
                test=test, test_chat_id=test_chat_id, post_type=post_type
            )
        except Exception as e:
            print("ERROR in _attempt_post: " + str(e))
            import traceback
            traceback.print_exc()
...
def check_flash_alerts_job():
    prices = fetch_prices()
    fear_greed_value, _ = fetch_fear_greed()
    check_flash_alerts(prices, fear_greed_value)
            ...
            with urllib.request.urlopen(req, timeout=35) as response:
                ...
                for update in result.get("result", []):
                    ...
                    if text.startswith("/post"):
                        ...
                        threading.Thread(
                            target=run_bot,
                            kwargs={"test": True, "test_chat_id": chat_id},
                            daemon=True,
                        ).start()
                    elif text.startswith("/recap"):
                        send_message_to_chat(chat_id, "Weekly recap incoming...")
                        threading.Thread(
                            target=run_weekly_recap, daemon=True
                        ).start()
                    elif text.startswith("/start"):
                        send_message_to_chat(
                            chat_id,
                            "Raw Brief Bot v3 active.\n\n"

...
def format_with_claude(articles, prices, fear_greed_value, fear_greed_class, test=False):
    ...
    prompt = "Market signal bot. Write like a trader texting his group. Direct. Everything in English.\n\n"
    prompt += "Rising DXY = bad for crypto/gold. Falling DXY = good for crypto/gold.\n\n"
    prompt += "SIGNAL LABEL RULES:\n"
    prompt += "🟢 BUY — clear upside setup with entry, take profit and stop loss\n"
    prompt += "🔴 SELL — clear downside setup with entry, take profit and stop loss\n"
    prompt += "🟡 WAIT — unclear or conflicting signals\n\n"
    prompt += "GOOD EXAMPLES:\n"
    prompt += "🟢 BUY ETH — holds $2,200. TP: $2,600. SL: $2,050. Last 3x: +18% in 2 weeks.\n"
    prompt += "🔴 SELL BTC — loses $75k + DXY rising. TP: $68k. SL: $77k. Historical: -12% in 72h.\n"
    prompt += "🟡 WAIT Gold — between $4,750-$4,850. No direction until Hormuz resolves.\n\n"
    ...
    prompt += "——————————————\n"
    prompt += "CRYPTO\n[SIGNAL] [asset] — [reason]. TP: $[price]. SL: $[price]. [historical ref]\n[SIGNAL] [asset] — [reason]. TP: $[price]. SL: $[price]. [historical ref]\n"
    prompt += "——————————————\n"
    prompt += "STOCKS\n[SIGNAL] [asset] — [reason]. TP: [price]. SL: [price]. [historical ref]\n[SIGNAL] [asset] — [reason]. TP: [price]. SL: [price]. [historical ref]\n"
    prompt += "——————————————\n"
    prompt += "COMMODITIES\n[SIGNAL] [asset] — [reason]. TP: $[price]. SL: $[price]. [historical ref]\n[SIGNAL] [asset] — [reason]. TP: $[price]. SL: $[price]. [historical ref]\n"
    prompt += "——————————————\n"
    prompt += "⚡ Key Level: [asset] $[price] — [breaks above/below = what happens, max 8 words]\n"
    prompt += "——————————————\n"
    prompt += "Not financial advice. DYOR. 🌲\n\n"
    prompt += "STRICT RULES:\n"
    prompt += "1. Every BUY and SELL signal MUST include TP (take profit) and SL (stop loss)\n"
    prompt += "2. WAIT signals — no TP/SL needed, just explain why unclear\n"
    prompt += "3. TP and SL based on nearest key support/resistance levels\n"
    prompt += "4. Every bullet must include historical reference (last Nx, before Yevent, etc)\n"
    prompt += "5. Crypto section: BTC and ETH only\n"
    prompt += "6. No URLs, no markdown, no source names\n"
    prompt += "7. Everything in English\n\n"
...
def generate_weekly_recap(prices, fear_greed_value, fear_greed_class):
    ...
    prompt = "Sharp market analyst. Weekly recap. Direct and actionable. Everything in English.\n\n"
    ...
    prompt += "——————————————\n"
    prompt += "Next Week — Watch For\n-> [key level or event to watch] [emoji]\n-> [key level or event to watch] [emoji]\n-> [key level or event to watch] [emoji]\n"
    prompt += "——————————————\n"
    prompt += "Overall Signal: [🟢 BUY THE DIP / 🔴 REDUCE EXPOSURE / 🟡 HOLD AND WAIT]\n"
    prompt += "——————————————\n"
    prompt += "Not financial advice. DYOR. 🌲\n\n"
    prompt += "RULES: Max 15 words per bullet. Specific prices. English only.\n\n"

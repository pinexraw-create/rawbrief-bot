...
Използва psycopg2 (sync) — APScheduler е sync scheduler.
"""
...
def save_prediction(
    ...
    """Записва предикция, връща id."""
    sql = """
        INSERT INTO predictions
            (asset, direction, entry_price, take_profit, stop_loss,
             confidence, rr_ratio, session)
    ...
    with get_conn() as conn:
        with conn.cursor() as cur:
            cur.execute(sql, (
                asset, direction, entry_price, take_profit,
                stop_loss, confidence, rr_ratio, session,
            ))
            row = cur.fetchone()
            return row[0]
...
def get_unverified_predictions(older_than_hours: int = 8) -> list:
    """Връща предикции без верификация, по-стари от N часа."""
    sql = """
...
def archive_old_predictions() -> int:
    """Архивира предикции по-стари от PREDICTION_HISTORY_DAYS дни. Връща броя."""
    sql = """
        UPDATE predictions
        SET archived = TRUE
...
def get_predictions_for_period(days: int = 7) -> list:
    """За weekly recap — верифицирани предикции за последните N дни."""
    sql = """
...
def get_weights(asset: str) -> dict:
    """Връща теглата за актив. Ако няма запис — default 1.0."""
    sql = """
...
def upsert_weights(asset: str, weights: dict) -> None:
    ...
    with get_conn() as conn:
        with conn.cursor() as cur:
            cur.execute(sql, (
                asset,
                weights.get("rsi_weight", 1.0),
                weights.get("fear_greed_weight", 1.0),
                weights.get("funding_weight", 1.0),
                weights.get("sr_weight", 1.0),
                weights.get("ema_weight", 1.0),
                weights.get("volume_weight", 1.0),
            ))


# ──────────────────────────────────────────────
# PRICE CACHE (persistent fallback)
# ──────────────────────────────────────────────
...
def get_cached_price(asset: str) -> Optional[float]:
    sql = "SELECT price FROM price_cache WHERE asset = %s"
    with get_conn() as conn:
        with conn.cursor() as cur:
            cur.execute(sql, (asset,))
            row = cur.fetchone()
...
def get_all_cached_prices() -> dict:
    sql = "SELECT asset, price FROM price_cache"
    with get_conn() as conn:
        with conn.cursor() as cur:
            cur.execute(sql)
...
def log_flash_alert(asset: str, direction: str, price: float, pct_change: float) -> None:
    sql = """
        INSERT INTO flash_alert_log (asset, direction, price, pct_change)
        VALUES (%s, %s, %s, %s)
    """
    with get_conn() as conn:
        with conn.cursor() as cur:
...
def get_recent_flash_alerts(asset: str, within_minutes: int = 60) -> list:
    """Проверява дали вече е пратен flash alert за актива наскоро."""
    sql = """
...
# ──────────────────────────────────────────────
# MORNING SNAPSHOT (за price movement context)
# ──────────────────────────────────────────────
def save_session_snapshot(session: str, prices: dict) -> None:
    """Записва snapshot на цените за дадена сесия (morning/midday)."""
    sql = """
        INSERT INTO price_cache (asset, price, updated_at)
        VALUES (%s, %s, NOW())
        ON CONFLICT (asset) DO UPDATE SET
            ...
            updated_at = NOW()
    """
    # Използваме отделна таблица чрез JSON в price_cache с prefix

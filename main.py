...
def send_message(text: str, channel_id: str = TELEGRAM_CHANNEL_ID) -> bool:
    """Праща съобщение в Telegram. PTB 21.x е async — използваме нов event loop."""
    ...
    for i, part in enumerate(parts):
        try:
            ...
            logger.error("Failed to send message part %d: %s", i + 1, e)
            success = False
    ...
    try:
        ...
        for asset, threshold in FLASH_THRESHOLDS.items():
            ...
            if not _is_volume_confirmed(asset):
                logger.info("Flash alert %s skipped — not volume-confirmed", asset)
                continue

            direction = "up" if pct_change > 0 else "down"
            logger.info("FLASH ALERT: %s %.2f%% to %.4f", asset, pct_change, current)
...
def build_scheduler() -> BlockingScheduler:
    ...
    scheduler.add_job(
        ...
        id="morning_post", name="Morning Post", misfire_grace_time=300,
    )
    scheduler.add_job(
        job_midday_post,
        CronTrigger(hour=MIDDAY_HOUR, minute=0, timezone="UTC"),
        id="midday_post", name="Midday Post", misfire_grace_time=300,
    )
    scheduler.add_job(
        job_evening_post,
        CronTrigger(hour=EVENING_HOUR, minute=0, timezone="UTC"),
        id="evening_post", name="Evening Post", misfire_grace_time=300,
    )
    scheduler.add_job(
        job_flash_alerts,
        "interval", minutes=5,
        id="flash_alerts", name="Flash Alerts", misfire_grace_time=60,
    )
    scheduler.add_job(
        job_weekly_recap,
        CronTrigger(day_of_week=WEEKLY_RECAP_DAY, hour=WEEKLY_RECAP_HOUR, minute=0, timezone="UTC"),
        id="weekly_recap", name="Weekly Recap", misfire_grace_time=600,
    )
...
# ──────────────────────────────────────────────
# COMMAND HANDLERS (/test, /post)
# ──────────────────────────────────────────────
async def cmd_test(update: telegram.Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """/test — праща сутрешен пост към теб (private)."""
    chat_id = str(update.effective_chat.id)
    if ADMIN_CHAT_ID and chat_id != ADMIN_CHAT_ID:
        await update.message.reply_text("❌ Not authorized.")
        return

    await update.message.reply_text("⏳ Building morning post with live data...")
    logger.info("/test called by %s", chat_id)

    try:
        post = build_morning_post()
        if post:
            parts = _split_message(post)
            for part in parts:
            ...
            await update.message.reply_text("❌ Post build failed — check logs.")
    except Exception as e:
        logger.error("/test failed: %s", e, exc_info=True)
        await update.message.reply_text(f"❌ Error: {e}")


async def cmd_post(update: telegram.Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """/post — праща сутрешен пост директно в канала."""
    chat_id = str(update.effective_chat.id)
    if ADMIN_CHAT_ID and chat_id != ADMIN_CHAT_ID:
        await update.message.reply_text("❌ Not authorized.")
        return

    await update.message.reply_text("⏳ Building and posting to channel...")
    logger.info("/post called by %s", chat_id)
...
def start_telegram_polling() -> None:
    """Стартира Telegram polling в отделен thread."""
    async def run():
        ...
        await app.start()
        logger.info("Telegram command polling started (/test, /post)")
        while True:
...
def main() -> None:
    ...
    if startup_env == "true":
        try:
            send_message(
                f"🟢 Raw Brief Bot is online.\n"
                f"Morning {MORNING_HOUR:02d}:00 | Midday {MIDDAY_HOUR:02d}:00 | Evening {EVENING_HOUR:02d}:00 UTC"
            )
        except Exception:
            pass

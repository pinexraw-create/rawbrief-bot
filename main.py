...
async def cmd_test(update: telegram.Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """
    /test — праща сутрешен пост с реални данни към ТЕБ (private chat).
    Само ADMIN_CHAT_ID може да го вика.
    """
...
async def cmd_post(update: telegram.Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """
    /post — праща сутрешен пост с реални данни директно в канала.
    Само ADMIN_CHAT_ID може да го вика.
    """
    chat_id = str(update.effective_chat.id)
    if ADMIN_CHAT_ID and chat_id != ADMIN_CHAT_ID:
...
def start_telegram_polling() -> None:
    """Стартира Telegram polling в отделен thread без signal handlers."""
    import asyncio
...
def main() -> None:
    ...
    # Стартираме Telegram command polling в отделен thread
